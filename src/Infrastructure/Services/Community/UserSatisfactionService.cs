...
public interface IUserSatisfactionService
{
    ...
    Task<FeedbackSummaryDto> GetFeedbackSummaryAsync(DateTime? startDate = null, DateTime? endDate = null);
    Task<List<UserFeedbackDto>> GetRecentFeedbackAsync(int count = 50);
...
public class UserSatisfactionService : IUserSatisfactionService
{
    ...
    private readonly ILogger<UserSatisfactionService> _logger;
    private readonly IAlertService _alertService;

    public UserSatisfactionService(
        ApplicationDbContext context,
        ILogger<UserSatisfactionService> logger,
    ...
    public async Task<UserSatisfactionDto> GetSatisfactionMetricsAsync(DateTime? startDate = null, DateTime? endDate = null)
    {
        try
        {
            ...
            _logger.LogInformation("Calculating user satisfaction metrics from {StartDate} to {EndDate}", start, end);
            ...
            var result = new UserSatisfactionDto
            {
    ...
    public async Task RecordUserFeedbackAsync(Guid userId, string feedbackType, int rating, string? comment = null)
    {
        try
        {
            ...
            _logger.LogInformation("Recording user feedback: User {UserId}, Type {FeedbackType}, Rating {Rating}", 
            ...
            _logger.LogInformation("User feedback recorded successfully: {FeedbackId}", feedback.Id);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error recording user feedback for user {UserId}", userId);
            throw;
        }
    }
    ...
    public async Task<FeedbackSummaryDto> GetFeedbackSummaryAsync(DateTime? startDate = null, DateTime? endDate = null)
    {
        try
        {
            var start = startDate ?? DateTime.UtcNow.AddDays(-30);
            ...
            // Get feedback data
            var feedbackQuery = _context.UserFeedback
    ...
    public async Task<List<UserFeedbackDto>> GetRecentFeedbackAsync(int count = 50)
    {
        try
        {
    ...
    public async Task TriggerSatisfactionSurveyAsync(Guid userId)
    {
        try
        {
            ...
            var recentSurvey = await _context.UserFeedback
            ...
            var surveyTrigger = new UserFeedback
            {
                Id = Guid.NewGuid(),
                UserId = userId,
            ...
            _context.UserFeedback.Add(surveyTrigger);
            await _context.SaveChangesAsync();
    ...
    private async Task<List<MetricDataPointDto>> GetSatisfactionTrendAsync(DateTime startDate, DateTime endDate)
    {
        try
        {
