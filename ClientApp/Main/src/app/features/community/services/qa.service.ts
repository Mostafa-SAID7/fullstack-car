...
import { Question, Answer, CreateQuestionRequest, CreateAnswerRequest, QuestionStatus, QuestionPriority } from '../../../core/models/qa.model';
...
export class QAService {
    ...
    private answersSubject = new BehaviorSubject<QAAnswer[]>([]);
        ...
        return this.qaQuestionService.getQuestions(filter).pipe(
            map(response => {
                if (response.succeeded && response.data) {
                    ...
                    return {
                        items: response.data.items.map(this.convertToLegacyQuestion),
        ...
        return this.qaQuestionService.getQuestionDetail(id).pipe(
            map(response => {
                if (response.succeeded && response.data) {
                    ...
                    return {
                        succeeded: true,
                        data: this.convertToLegacyQuestion(response.data),
        ...
        return this.qaAnswerService.createAnswer(request).pipe(
            map(response => {
                if (response.succeeded && response.data) {
                    ...
                    return {
                        succeeded: true,
                        data: this.convertToLegacyAnswer(response.data),
                        message: response.message,
                        errors: []
                    } as Result<Answer>;
                }
    ...
    private initializeRealtimeUpdates(): void {
        ...
        this.qaSignalRService.voteCreated$.subscribe(event => {
            ...
            if (event.contentType === 'Question') {
                this.updateQuestionVoteScoreRealtime(event.contentId, event.newVoteScore);
            } else {
                this.updateAnswerVoteScoreRealtime(event.contentId, event.newVoteScore);
            }
        });
    ...
    private convertToLegacyQuestion(question: any): Question {
        return {
            id: question.id,
            title: question.title,
            content: question.content || '',
            status: QuestionStatus.Open, // Default status
            priority: QuestionPriority.Normal, // Default priority
            viewsCount: question.viewCount || 0,
            upvotesCount: Math.max(0, question.voteScore || 0), // Assume positive votes are upvotes
            downvotesCount: Math.max(0, -(question.voteScore || 0)), // Assume negative votes are downvotes
            answersCount: question.answerCount || 0,
            hasAcceptedAnswer: !!question.acceptedAnswerId,
            acceptedAnswerId: question.acceptedAnswerId,
            tags: Array.isArray(question.tags) ? question.tags.join(',') : (question.tags as string) || '',
            bountyAmount: 0, // Default bounty
            userId: question.userId,
            categoryId: question.category, // Map category to categoryId
            createdAt: new Date(question.createdAt)
        } as Question;
    }

    private convertToLegacyAnswer(answer: QAAnswer): Answer {
        return {
            id: answer.id,
            content: answer.content,
            status: 0, // Default status
            upvotesCount: Math.max(0, answer.voteScore), // Assume positive votes are upvotes
            downvotesCount: Math.max(0, -answer.voteScore), // Assume negative votes are downvotes
    ...
    // Public getters for current state (following existing patterns)
