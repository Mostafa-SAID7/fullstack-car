...
public class QASearchService : IQASearchService
{
    ...
    private readonly QASearchOptions _options;
    
    // Cache keys
    private const string SEARCH_CACHE_PREFIX = "qa_search_";
    private const string CATEGORY_CACHE_KEY = "qa_categories_count";
    private const string TAG_CACHE_KEY = "qa_tags_count";
    private const string SEARCH_INDEX_KEY = "qa_search_index";
    private const string ANALYTICS_CACHE_KEY = "qa_search_analytics";

    public QASearchService(
        IApplicationDbContext context,
        IMemoryCache cache,
        ILogger<QASearchService> logger,
        IOptions<QASearchOptions> options)
    {
        _context = context;
        _cache = cache;
        _logger = logger;
    ...
    public async Task<Result<QASearchResultsDto>> AdvancedSearchAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                return Result<QASearchResultsDto>.Failure(questionsResult.ErrorMessage ?? "Error searching questions");
            }

            var results = new QASearchResultsDto
            {
            ...
            results.Metadata = new QASearchMetadataDto
            {
            ...
            return Result<QASearchResultsDto>.Success(results);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error performing advanced search");
            return Result<QASearchResultsDto>.Failure("An error occurred during advanced search");
        }
    }
    ...
    public async Task<Result<QASearchAnalyticsDto>> GetSearchAnalyticsAsync(
        DateTime fromDate,
        DateTime toDate,
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var analytics = new QASearchAnalyticsDto
            {
                TotalSearches = 1250,
                UniqueUsers = 340,
            ...
            return Result<QASearchAnalyticsDto>.Success(analytics);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting search analytics");
            return Result<QASearchAnalyticsDto>.Failure("An error occurred while getting search analytics");
        }
    }
    ...
    public async Task<Result> RebuildSearchIndexAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var indexEntries = new List<QASearchIndexEntry>();
    ...
    private async Task<QASearchIndexEntry?> CreateSearchIndexEntryAsync(Guid contentId, string contentType, CancellationToken cancellationToken)
    {
    ...
    private async Task<QASearchIndexEntry> CreateQuestionIndexEntryAsync(Question question, CancellationToken cancellationToken)
    {
    ...
    private async Task<QASearchIndexEntry> CreateAnswerIndexEntryAsync(Answer answer, CancellationToken cancellationToken)
    {
        var userReputation = await _context.UserReputations
    ...
    private async Task UpdateInMemorySearchIndexAsync(QASearchIndexEntry indexEntry, CancellationToken cancellationToken)
    {
        // Get current search index from cache
        var searchIndex = _cache.Get<List<QASearchIndexEntry>>(SEARCH_INDEX_KEY) ?? new List<QASearchIndexEntry>();
    ...
    private void InvalidateSearchCaches(string contentType, QASearchIndexEntry indexEntry)
    {
    ...
    private List<QASearchTrendDto> GenerateSearchTrends(DateTime fromDate, DateTime toDate)
    {
        var trends = new List<QASearchTrendDto>();
        ...
        while (currentDate <= toDate.Date)
        {
            ...
            trends.Add(new QASearchTrendDto
            {
                Date = currentDate,
