...
public interface IQAHealthMonitoringService
{
    ...
    Task<QAPerformanceMetricsDto> GetPerformanceMetricsAsync();
    Task<List<QASystemAlertDto>> GetSystemAlertsAsync();
    Task<QAUserSatisfactionDto> GetUserSatisfactionMetricsAsync();
    Task RecordPerformanceMetricAsync(string metricName, double value, string category = "General");
    Task TriggerHealthCheckAsync();
}

public class QAHealthMonitoringService : IQAHealthMonitoringService
{
    private readonly ApplicationDbContext _context;
    private readonly ILogger<QAHealthMonitoringService> _logger;
    private readonly AnalyticsSettings _analyticsSettings;
    private readonly IQAConnectionManager _connectionManager;

    public QAHealthMonitoringService(
        ApplicationDbContext context,
        ILogger<QAHealthMonitoringService> logger,
        IOptions<AnalyticsSettings> analyticsSettings,
        IQAConnectionManager connectionManager)
    {
        _context = context;
        _logger = logger;
    ...
    public async Task<QASystemHealthDto> GetSystemHealthAsync()
    {
        try
        {
            var stopwatch = Stopwatch.StartNew();

            // Get basic QA system metrics
            ...
            var recentVotes = await _context.QAVotes.CountAsync(v => v.CreatedAt >= yesterday);
            ...
            var healthDto = new QASystemHealthDto
            {
                Status = healthStatus,
                Timestamp = DateTime.UtcNow,
                CheckDurationMs = stopwatch.ElapsedMilliseconds,
                Metrics = new QAHealthMetricsDto
                {
                ...
                Dependencies = new QADependencyHealthDto
                {
            ...
            _logger.LogInformation("QA System health check completed in {Duration}ms. Status: {Status}", 
        ...
        {
            _logger.LogError(ex, "Error performing QA system health check");
            return new QASystemHealthDto
            {
                Status = "Critical",
                Timestamp = DateTime.UtcNow,
                CheckDurationMs = 0,
    ...
    public async Task<QAPerformanceMetricsDto> GetPerformanceMetricsAsync()
    {
        try
        {
            ...
            var voteMetrics = await _context.QAVotes
            ...
            var searchPerformance = new QASearchPerformanceDto
            {
            ...
            return new QAPerformanceMetricsDto
            {
                Timestamp = DateTime.UtcNow,
                QuestionTrends = questionMetrics.Select(m => new QAMetricDataPointDto 
                { 
                    Date = m.Date, 
                    Value = m.Count 
                }).ToList(),
                AnswerTrends = answerMetrics.Select(m => new QAMetricDataPointDto 
                { 
                    Date = m.Date, 
                    Value = m.Count 
                }).ToList(),
                VoteTrends = voteMetrics.Select(m => new QAMetricDataPointDto 
                { 
                    Date = m.Date, 
                    Value = m.Count 
                }).ToList(),
        ...
        {
            _logger.LogError(ex, "Error getting QA performance metrics");
            throw;
        }
    }

    public async Task<List<QASystemAlertDto>> GetSystemAlertsAsync()
    {
        var alerts = new List<QASystemAlertDto>();
        ...
        try
        {
            ...
            {
                alerts.Add(new QASystemAlertDto
                {
                    Id = Guid.NewGuid(),
                    Type = "Warning",
            ...
            var activeExperts = await _context.QAExperts
        ...
        {
            _logger.LogError(ex, "Error getting QA system alerts");
            alerts.Add(new QASystemAlertDto
            {
                Id = Guid.NewGuid(),
                Type = "Error",
    ...
    public async Task<QAUserSatisfactionDto> GetUserSatisfactionMetricsAsync()
    {
        try
        {
            ...
            return new QAUserSatisfactionDto
            {
                Timestamp = DateTime.UtcNow,
    ...
    public async Task TriggerHealthCheckAsync()
    {
        try
        {
            _logger.LogInformation("Triggering QA system health check");
            ...
            {
                _logger.LogWarning("QA System has {Count} critical alerts", criticalAlerts.Count);
            ...
            _logger.LogInformation("QA system health check completed. Status: {Status}", health.Status);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error during QA health check trigger");
        }
    }
    ...
    private async Task<QASystemLoadDto> GetSystemLoadMetricsAsync()
    {
        try
        {
    ...
    private async Task<List<QAMetricDataPointDto>> GetSatisfactionTrendAsync()
    {
        try
        {
            var last7Days = DateTime.UtcNow.AddDays(-7);
