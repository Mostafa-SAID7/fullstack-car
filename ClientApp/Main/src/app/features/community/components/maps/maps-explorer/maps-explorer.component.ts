...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, LocationCardComponent, PaginationComponent],
  template: `
    <div class="p-4 lg:p-8 max-w-[1600px] mx-auto animate-fade-in space-y-6">
      ...
      <div class="bg-white dark:bg-[#111] rounded-[2.5rem] p-6 shadow-xl border border-black/5 dark:border-white/5">
        <form [formGroup]="searchForm" class="flex flex-col md:flex-row gap-4 mb-0">
          ...
          <div class="relative flex-grow group">
            ...
            <input formControlName="searchTerm" type="text" placeholder="Search locations..."
        ...
        <div *ngIf="showFilters" class="pt-6 animate-fade-in">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="flex flex-col">
              <label class="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-3 ml-4 opacity-70">Location Type</label>
              <div class="relative">
                ...
                <select formControlName="type"
                  ...
                  <option *ngFor="let type of locationTypes" [ngValue]="type.value">{{ type.name }}</option>
                </select>
      ...
      <div class="bg-white dark:bg-[#111] rounded-[2.5rem] p-6 lg:p-8 shadow-xl border border-black/5 dark:border-white/5 min-h-[500px]">
        ...
        <div *ngIf="!loading && locations.length === 0" class="py-24 text-center">
          ...
          <h3 class="text-xl font-black text-foreground uppercase tracking-widest mb-2">No locations found</h3>
          <p class="text-muted-foreground font-bold text-xs uppercase tracking-widest">Try adjusting your filters or search terms</p>
        </div>
      </div>
...
export class MapsExplorerComponent implements OnInit {
  ...
  locationTypes = [
    { name: 'Showroom', value: LocationType.Showroom },
    { name: 'Service Center', value: LocationType.ServiceCenter },
    { name: 'Spare Parts', value: LocationType.SpareParts },
    { name: 'Charging Station', value: LocationType.ChargingStation },
    { name: 'Gas Station', value: LocationType.GasStation },
    { name: 'Other', value: LocationType.Other }
  ];
  ...
  handleCheckIn(location: Location): void {
    ...
    // Future: Implement actual check-in logic
  }
}
