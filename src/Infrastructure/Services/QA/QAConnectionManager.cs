...
/// </summary>
public interface IQAConnectionManager
{
    ...
    // Connection tracking methods used by QAHub
...
public class QAConnectionManager : BackgroundService, IQAConnectionManager
{
    private readonly IHubContext<QAHub, IQAHub> _hubContext;
    private readonly IQAHubService _qaHubService;
    private readonly ILogger<QAConnectionManager> _logger;
    ...
    public void TrackConnection(string connectionId, Guid userId, string userName, string userAgent = "")
    {
        ...
        _logger.LogInformation("QA Connection Manager: User {UserName} ({UserId}) connected with {ConnectionId}", 
    ...
    public void TrackDisconnection(string connectionId, string reason = "")
    {
        if (_activeConnections.TryRemove(connectionId, out var connection))
        {
            ...
            _logger.LogInformation("QA Connection Manager: User {UserName} ({UserId}) disconnected from {ConnectionId}. Reason: {Reason}", 
    ...
    public void TrackGroupJoin(string connectionId, string groupName)
    {
        ...
        _logger.LogDebug("QA Connection Manager: Connection {ConnectionId} joined group {GroupName}", 
    ...
    public void TrackGroupLeave(string connectionId, string groupName)
    {
        ...
        _logger.LogDebug("QA Connection Manager: Connection {ConnectionId} left group {GroupName}", 
            connectionId, groupName);
    }
    ...
    public async Task DisconnectUserAsync(Guid userId, string reason)
    {
        if (_userConnections.TryGetValue(userId, out var connectionIds))
        {
            foreach (var connectionId in connectionIds.ToList())
            {
                ...
                {
                    _logger.LogError(ex, "QA Connection Manager: Failed to disconnect user {UserId} from connection {ConnectionId}", 
                        userId, connectionId);
                }
            }
        }

        _logger.LogInformation("QA Connection Manager: Disconnected user {UserId}. Reason: {Reason}", 
            userId, reason);
    }
    ...
    public async Task BroadcastSystemMessageAsync(string message, string level = "info")
    {
        try
        {
            ...
            _logger.LogInformation("QA Connection Manager: Broadcasted system message: {Message}", message);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "QA Connection Manager: Failed to broadcast system message");
            throw;
        }
    }
    ...
    public async Task CleanupStaleConnectionsAsync()
    {
        ...
        {
            _logger.LogInformation("QA Connection Manager: Cleaned up {Count} stale connections", 
    ...
    {
        _logger.LogInformation("QA Connection Manager background service started");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "QA Connection Manager: Error in background monitoring");
                await Task.Delay(TimeSpan.FromSeconds(30), stoppingToken);
            }
        }

        _logger.LogInformation("QA Connection Manager background service stopped");
    }
    ...
    private async Task MonitorConnectionHealthAsync()
    {
        ...
        // Log health status
        _logger.LogDebug("QA Connection Manager Health: {ActiveConnections} active, {StaleConnections} stale", 
        ...
        {
            _logger.LogWarning("QA Connection Manager: High stale connection count detected: {StaleConnections}/{ActiveConnections}", 
    ...
    private async void PerformHealthCheck(object? state)
    {
        ...
        {
            _logger.LogError(ex, "QA Connection Manager: Error during health check");
        }
    }
    ...
    private async void PerformCleanup(object? state)
    {
        ...
        {
            _logger.LogError(ex, "QA Connection Manager: Error during cleanup");
        }
    }
