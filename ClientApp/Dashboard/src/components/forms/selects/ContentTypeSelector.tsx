...
import { ChevronDown, FileText, MessageSquare, Image, Calendar, Users, Star, Flag, AlertTriangle } from 'lucide-react';
...
export type ContentType = 'posts' | 'comments' | 'media' | 'events' | 'groups' | 'reviews' | 'reports' | 'announcements';
...
export const ContentTypeSelector: React.FC<ContentTypeSelectorProps> = ({
  ...
  return (
    <div className={cn("relative", className)} ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute top-full mt-2 w-80 bg-card border border-border rounded-xl shadow-xl z-50 overflow-hidden">
          <div className="p-2">
              ...
              {contentTypes.map((type) => {
                ...
                return (
                  <button
                    key={type.id}
                    onClick={() => {
                      onTypeChange(type.id as ContentType);
                      setIsOpen(false);
                    }}
                    className={cn(
