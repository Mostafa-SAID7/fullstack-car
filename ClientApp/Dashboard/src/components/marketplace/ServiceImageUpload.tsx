 ...
 * Specialized component for uploading and managing service images
 */
...
export interface ServiceImageUploadProps {
  /** Service ID for image association */
  serviceId?: string;
...
export const ServiceImageUpload: React.FC<ServiceImageUploadProps> = ({
  ...
  const handleFileSelect = useCallback(async (files: File[]) => {
    ...
    for (let i = 0; i < filesToUpload.length; i++) {
      ...
      try {
        if (serviceId) {
          // Upload to backend if serviceId is available
          const result = await imageUploadService.uploadServiceImage(
            serviceId,
            file,
            uploadOptions,
          ...
          // Mock upload for form preview (when no serviceId)
          setTimeout(() => {
            setImages(prev => prev.map((img, idx) => 
              idx === imageIndex 
                ? { 
  ...
  }, [disabled, maxImages, images.length, serviceId, uploadOptions, onPrimaryImageChange]);
  ...
  const removeImage = useCallback(async (index: number) => {
    ...
    try {
      // Delete from backend if it has a URL and serviceId
      if (image.url && serviceId && !image.previewUrl) {
        await imageUploadService.deleteServiceImage(serviceId, image.url);
      }
  ...
  }, [images, serviceId, onPrimaryImageChange]);

  const setPrimaryImage = useCallback(async (index: number) => {
    const image = images[index];
    
    try {
      // Update backend if serviceId is available
      if (serviceId && image.url) {
        await imageUploadService.setPrimaryServiceImage(serviceId, image.url);
      }
      ...
      {canUploadMore && (
        ...
        >
          ...
          <h4 className="text-sm font-medium text-foreground mb-1">
            {isDragOver ? 'Drop images here' : 'Upload service images'}
          </h4>
