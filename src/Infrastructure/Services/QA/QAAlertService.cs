...
public class QAAlertService : IQAAlertService
{
    private readonly IHubContext<NotificationHub> _notificationHub;
    private readonly ILogger<QAAlertService> _logger;
    private readonly AnalyticsSettings _analyticsSettings;

    public QAAlertService(
        IHubContext<NotificationHub> notificationHub,
        ILogger<QAAlertService> logger,
    ...
    public async Task SendHealthAlertAsync(QASystemAlertDto alert)
    {
        try
        {
            _logger.LogInformation("Sending QA health alert: {Title} (Severity: {Severity})", 
                alert.Title, alert.Severity);

            // Send to admin group following existing notification patterns
            await _notificationHub.Clients.Group("admin").SendAsync("ReceiveQAHealthAlert", new
            {
                Id = alert.Id,
                Type = "QAHealthAlert",
                Title = alert.Title,
            ...
            {
                await _notificationHub.Clients.Group("moderator").SendAsync("ReceiveQAHealthAlert", new
                {
                    Id = alert.Id,
                    Type = "QACriticalAlert",
                    Title = $"🚨 Critical QA Alert: {alert.Title}",
                    Message = alert.Message,
                    Severity = alert.Severity,
                    Category = alert.Category,
                    Timestamp = alert.CreatedAt,
            ...
            _logger.LogInformation("QA health alert sent successfully: {AlertId}", alert.Id);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error sending QA health alert: {AlertId}", alert.Id);
        }
    }
    ...
    public async Task SendPerformanceAlertAsync(string metricName, double value, double threshold, string severity)
    {
        try
        {
            var alertTitle = $"QA Performance Alert: {metricName}";
            var alertMessage = $"Metric '{metricName}' value ({value:F2}) has exceeded threshold ({threshold:F2})";

            _logger.LogWarning("QA Performance Alert: {MetricName} = {Value} (Threshold: {Threshold}, Severity: {Severity})", 
                metricName, value, threshold, severity);

            var notification = new
            {
                Id = Guid.NewGuid(),
                Type = "QAPerformanceAlert",
                Title = alertTitle,
            ...
            if (severity == "Critical")
            {
                await _notificationHub.Clients.Groups(new[] { "admin", "moderator" })
                    .SendAsync("ReceiveQAPerformanceAlert", notification);
            }
            else
            {
    ...
    public async Task SendExpertAvailabilityAlertAsync(string category, int availableExperts, int minimumRequired)
    {
        try
        {
            ...
            _logger.LogWarning("QA Expert Availability Alert: {Category} has {Available} experts (minimum: {Required})", 
                category, availableExperts, minimumRequired);

            var notification = new
            {
                Id = Guid.NewGuid(),
                Type = "QAExpertAvailabilityAlert",
                Title = alertTitle,
                Message = alertMessage,
                Severity = severity,
                Category = "Experts",
            ...
            await _notificationHub.Clients.Groups(new[] { "admin", "moderator" })
                .SendAsync("ReceiveQAExpertAlert", notification);
    ...
    public async Task SendSystemStatusChangeAsync(string oldStatus, string newStatus, string reason)
    {
        try
        {
            ...
            var alertTitle = $"QA System Status Changed: {oldStatus} → {newStatus}";
            var alertMessage = $"QA system status changed from {oldStatus} to {newStatus}. Reason: {reason}";

            _logger.LogInformation("QA System Status Change: {OldStatus} → {NewStatus} (Reason: {Reason})", 
                oldStatus, newStatus, reason);

            var notification = new
            {
                Id = Guid.NewGuid(),
                Type = "QASystemStatusChange",
                Title = alertTitle,
                Message = alertMessage,
                Severity = severity,
                Category = "System",
            ...
            await _notificationHub.Clients.Group("admin")
                .SendAsync("ReceiveQASystemStatusChange", notification);
    ...
    public async Task NotifyAdministratorsAsync(string title, string message, string severity = "Info")
    {
        try
        {
            _logger.LogInformation("Sending QA administrator notification: {Title} (Severity: {Severity})", 
                title, severity);

            var notification = new
            {
                Id = Guid.NewGuid(),
                Type = "QAAdminNotification",
                Title = title,
                Message = message,
                Severity = severity,
            ...
            await _notificationHub.Clients.Group("admin")
                .SendAsync("ReceiveQAAdminNotification", notification);
...
public class QAHealthMonitorBackgroundService : BackgroundService
{
    ...
    private readonly ILogger<QAHealthMonitorBackgroundService> _logger;
    private readonly TimeSpan _checkInterval = TimeSpan.FromMinutes(5); // Check every 5 minutes

    public QAHealthMonitorBackgroundService(
        IServiceProvider serviceProvider,
        ILogger<QAHealthMonitorBackgroundService> logger)
    {
    ...
    {
        _logger.LogInformation("QA Health Monitor Background Service started");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var healthService = scope.ServiceProvider.GetRequiredService<IQAHealthMonitoringService>();
                var alertService = scope.ServiceProvider.GetRequiredService<IQAAlertService>();
                ...
                _logger.LogDebug("QA Health Monitor check completed. Status: {Status}, Alerts: {AlertCount}", 
        ...
        _logger.LogInformation("QA Health Monitor Background Service stopped");
    }

    private async Task CheckPerformanceThresholds(IQAHealthMonitoringService healthService, IQAAlertService alertService)
    {
        try
        {
            var health = await healthService.GetSystemHealthAsync();
        ...
        {
            _logger.LogError(ex, "Error checking QA performance thresholds");
        }
    }
}