-- QA System Performance Optimization Script (Fixed Version)
...
PRINT 'Starting QA Performance Optimization (Fixed Version)...'
...
-- Index for trending questions (high activity) - Fixed WHERE clause
...
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'QAVotes')
    UPDATE STATISTICS [QAVotes] WITH FULLSCAN

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'UserReputation')
    UPDATE STATISTICS [UserReputation] WITH FULLSCAN

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'QAExperts')
    UPDATE STATISTICS [QAExperts] WITH FULLSCAN

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'QAAnalytics')
    UPDATE STATISTICS [QAAnalytics] WITH FULLSCAN

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'QAUserActivity')
    UPDATE STATISTICS [QAUserActivity] WITH FULLSCAN

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'QACategories')
    UPDATE STATISTICS [QACategories] WITH FULLSCAN

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'QATags')
    UPDATE STATISTICS [QATags] WITH FULLSCAN

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'QuestionTags')
    UPDATE STATISTICS [QuestionTags] WITH FULLSCAN

PRINT 'Statistics updated successfully'

-- =============================================
-- Query Store Configuration (if supported)
-- =============================================

PRINT 'Configuring Query Store for performance monitoring...'

-- Check if Query Store is supported and enable if not already enabled
IF EXISTS (SELECT * FROM sys.databases WHERE name = DB_NAME() AND is_query_store_on = 0)
BEGIN
    BEGIN TRY
    ...
    END TRY
    BEGIN CATCH
        PRINT 'Query Store configuration failed - may not be supported in this SQL Server edition'
    END CATCH
END
ELSE
BEGIN
    PRINT 'Query Store already enabled or not supported'
END
...
BEGIN
    ...
    CREATE INDEX IX_QAPerformanceBaseline_Category_CreatedAt ON QAPerformanceBaseline(Category, CreatedAt DESC)
    PRINT 'Created QA performance baseline table'
END

-- Insert baseline metrics (only if table is empty)
IF NOT EXISTS (SELECT * FROM QAPerformanceBaseline)
BEGIN
...
ELSE
BEGIN
    PRINT 'Performance baseline metrics already exist'
END
...
PRINT '- Configured Query Store for monitoring (if supported)'
