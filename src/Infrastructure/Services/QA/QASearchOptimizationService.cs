...
public interface IQASearchOptimizationService
{
...
public class QASearchOptimizationService : IQASearchOptimizationService
{
    private readonly IAdvancedCacheService _cacheService;
    private readonly ILogger<QASearchOptimizationService> _logger;
    private readonly QASearchPerformanceOptions _options;
    ...
    public QASearchOptimizationService(
        IAdvancedCacheService cacheService,
        ILogger<QASearchOptimizationService> logger,
        IOptions<QASearchPerformanceOptions> options)
    {
    ...
    public async Task InvalidateSearchCacheAsync(string pattern, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            await _cacheService.RemoveByPatternAsync($"qa:search:{pattern}*");
    ...
    private async Task PrewarmSearchTermAsync(string searchTerm, CancellationToken cancellationToken)
    {
        try
        {
            var cacheKey = $"qa:search:prewarm:{GenerateQueryHash(searchTerm)}";
...
public class QASearchPerformanceOptions
{
    public const string SectionName = "QASearchPerformance";
